    ...
    <div style={{ maxWidth: 800, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem", marginBottom: "1.5rem" }}>
          {[
            ...
            <div key={s.label} className={`glass-card stat-card-${s.accent}`} style={{ padding: "1rem 1rem 1rem 1.25rem" }}>
              <div style={{ fontSize: "0.7rem", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 600, marginBottom: "0.375rem" }}>
                {s.label}
              </div>
              <div className="num" style={{ fontSize: "1.5rem", fontWeight: 700, color: "var(--text-primary)" }}>{s.value}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {history.length === 0 ? (
        <div className="glass-card" style={{ padding: "3rem 2rem", textAlign: "center" }}>
          ...
          <h3 style={{ fontWeight: 600, color: "var(--text-primary)", marginBottom: "0.5rem" }}>Belum ada riwayat</h3>
        ...
        <div className="glass-card" style={{ overflow: "hidden" }}>
          <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
            {history.map((h, i) => (
              <li key={h.id} style={{ borderBottom: i < history.length - 1 ? "1px solid var(--border)" : "none" }}>
                <Link
